<template>
   <aside>
       <FloatLabel>
           <InputText :disabled="processing || form.processing || disabled" v-if="form && name" :type="type" :class="{'p-invalid':hasError()}" v-model="form[name]"/>
           <InputText v-else :value="modelValue" @input="updateValue" :type="type" :class="{'p-invalid':hasError()}"/>
           <label>
               {{ label ?? $t('column.' + name) }}
